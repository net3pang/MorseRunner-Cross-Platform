...
**PR #4（[wzh06 / Lewis](https://github.com/wzh06)）**

1. **输入光标乱跳修复**：改用 field editor 的原始编辑路径处理大小写和输入限制，保留插入位置与选区；回车后光标定位到呼号末尾，不再因格式化整个文本而跳动或全选。
2. **计时结束未停止修复**：到达训练时限时立即停止所有台站的发送、清空音频队列并切换到停止状态；后续音频回调只返回静音，避免计时结束后仍继续发射或界面保持运行。
3. **未完成 QSO 不参与计分**：训练时间结束时，正在进行中的 QSO 会保留为未验证状态并标记为 `NIL`/错误，不会计入 Verified 统计。
4. **输入与发送稳定性提升**：修复发送中修改呼号时的数组越界、空呼号片段导致的无效范围崩溃，并新增回归测试。
5. **Universal 打包脚本**：新增 `make-app-universal.sh`，分别构建 Intel `x86_64` 与 Apple Silicon `arm64` 后合成为通用 `.app`，并增加临时构建、签名、架构和 plist 校验。
...
5. **Touch Bar 显示问题修复**：WPM 输入框改用无候选词条的 `NoCandidateTextField`；
   输入框 field editor（统一大写、禁中文输入、关闭自动补全）扩展到我的呼号、
...
| [wzh06 / Lewis](https://github.com/wzh06) | PR #3 结果窗 QSO 计数、发送中改呼号、发送延迟优化与崩溃修复；PR #4 光标、超时停止、计分和通用打包修复 |
